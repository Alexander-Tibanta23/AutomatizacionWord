...
def load_data():
    path = "data/basedatosNatural.xlsx"
    workbook = openpyxl.load_workbook(path)
    sheet = workbook.active

    # Configurar encabezados de columna, excluyendo el ID
    for col_name in next(sheet.values)[1:]:  # Excluir el primer encabezado (ID)
        treeview.heading(col_name, text=col_name)
        treeview.column(col_name, anchor="center")

    # Cargar datos, excluyendo la columna del ID
    for row in sheet.iter_rows(min_row=2, values_only=True):
        treeview.insert('', tk.END, values=row[1:])  # Excluir el ID al cargar en Treeview

def load_detail_data(id):
    new_excel_path = "data/detallesBasedatosNatural.xlsx"
    try:
...
def insert_row():
    ...
    # Generar un nuevo ID incrementable
    if sheet.max_row == 1:  # Si solo está el encabezado
        id = 1
    else:
        # El ID incrementable estará en la columna 1 (ajusta según sea necesario)
        last_id = sheet.cell(row=sheet.max_row, column=1).value
        id = last_id + 1

    row_values = [id, judge, name, ruc, id_number, lawyer]
    sheet.append(row_values)
    workbook.save(path)

    # Insertar la nueva fila en el Treeview SIN el ID
    treeview.insert('', tk.END, values=row_values[1:])
...
def update_excel_with_details(id_number, new_values):
    filepath = "data/detallesBasedatosNatural.xlsx"
    try:
...
def save_changes(treeview):
    ...
    if selected_item_for_editing:
        ...
        treeview.item(selected_item_for_editing, values=[judge, name, ruc, id_number, lawyer])
...
def on_double_click(event):
    ...
    selected_values = treeview.item(selected_item, 'values')  # Obtiene los valores del ítem seleccionado
    ...
    # Asigna los valores seleccionados a variables individuales
    judge, name, ruc, id_number, lawyer = selected_values

    # Buscar el `unique_id` en el archivo Excel usando los valores seleccionados
    path = "data/basedatosNatural.xlsx"
    workbook = openpyxl.load_workbook(path)
    sheet = workbook.active
    unique_id = None

    # Buscar el ID único correspondiente a los datos seleccionados
    for row in sheet.iter_rows(min_row=2, values_only=True):
        if list(row[1:]) == list(selected_values):  # Comparar los datos excluyendo el ID
            unique_id = row[0]  # Obtener el ID único (columna 1 en Excel)
            break

    if unique_id:
        # Llama a la función que maneja la apertura de una ventana con los detalles, pasando el ID único
        open_details_window(judge, name, ruc, id_number, lawyer, unique_id)  # Pasamos tanto el id_number como el unique_id
    else:
        print("ID no encontrado para el registro seleccionado.")
...
def open_details_window(judge, name, ruc, id_number, lawyer, unique_id):

    load_detail_data(unique_id)
    ...
    if unique_id in details_storage:
        for detail in details_storage[unique_id]:
    ...
    def delete_detail_from_excel(unique_id, detail_to_delete):
        filepath = "data/detallesBasedatosNatural.xlsx"
        wb = load_workbook(filename=filepath)
        sheet = wb.active
        
        for row in range(2, sheet.max_row + 1):
            if sheet.cell(row=row, column=1).value == unique_id and sheet.cell(row=row, column=2).value == detail_to_delete[0]:
                sheet.delete_rows(row)
                break
        
        wb.save(filename=filepath)

    def update_detail_in_excel(unique_id, old_detail, new_detail):
        filepath = "data/Providencias/detallesBasedatosNatural.xlsx"
        wb = load_workbook(filename=filepath)
        sheet = wb.active
        
        for row in range(2, sheet.max_row + 1):
            if sheet.cell(row=row, column=1).value == unique_id and sheet.cell(row=row, column=2).value == old_detail[0]:
    ...
    def delete_detail(detail_treeview, unique_id):
        selected_item = detail_treeview.selection()[0]
        if selected_item:
            detail_to_delete = detail_treeview.item(selected_item, 'values')
            delete_detail_from_excel(unique_id, detail_to_delete)
            detail_treeview.delete(selected_item)
            # Actualizar details_storage
            details_storage[unique_id] = [detail for detail in details_storage[unique_id] if detail[0] != detail_to_delete[0]]
    ...
    def update_detail(detail_treeview, unique_id, entries):
        selected_item = detail_treeview.selection()[0]
        if selected_item:
            ...
            new_valor_capital = f"{valor_capital:,.2f}".replace('.', ',')
            new_valor_liquidacion = f"{valor_liquidacion:,.2f}".replace('.', ',')
            
            # Construir los nuevos valores del detalle a partir de los widgets de entrada
            new_values = [entries[0].get(), name, ruc, id_number, entries[1].get(), new_valor_capital, new_valor_liquidacion, valor_30, lawyer, judge]
            old_detail = detail_treeview.item(selected_item, 'values')
            
            # Actualizar Excel
            update_detail_in_excel(unique_id, old_detail, new_values)  # Usar `unique_id` como referencia
            
            # Actualizar en el Treeview
            detail_treeview.item(selected_item, values=new_values)
            
            # Actualizar en `details_storage`
            details_storage[unique_id] = [new_values if detail[0] == old_detail[0] else detail for detail in details_storage[unique_id]]


    def update_all_details(detail_treeview, unique_id, entries):
        for selected_item in detail_treeview.selection():
            ...
            # Construir los nuevos valores del detalle a partir de los widgets de entrada
            new_values = [entries[0].get(), name, ruc, id_number, entries[1].get(), entries[2].get(), new_valor_liquidacion, valor_30, lawyer, judge]
            old_detail = detail_treeview.item(selected_item, 'values')
            ...
            # Actualizar en `details_storage`
            details_storage[unique_id] = new_values
    ...
    template_path = get_resource_path("templates/at-plantilla-DocumentoNatural.docx")
    ...
    def create_word_document(treeview, combo, gender_combo, day_combo, month_combo, hour_combo, minute_combo):
        ...
        template_path = "templates/at-plantilla-DocumentoNatural.docx"
        ...
        doc = DocxTemplate("templates/at-plantilla-DocumentoNatural.docx")
        doc.render(context)

        # Crear la estructura de carpetas
        base_folder = "NATURAL/PROVIDENCIAS"
    ...
    def export_to_excel(treeview, serial_entry, date_entry, filename="exported_data.xlsx"):
        ...
        # Crear la estructura de carpetas
        base_folder = "NATURAL/BASE DATOS"
        folder_name = datetime.now().strftime("%B-%Y").upper()
        day_folder = datetime.now().strftime("%d de %B").upper()
        complete_path = os.path.join(base_folder, folder_name, day_folder)
        if not os.path.exists(complete_path):
        ...
        filename = f"{complete_path}/Data_{context_general['nombre'].replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    ...
    ttk.Button(inputs_frame, text="Actualizar", command=lambda: update_detail(
        detail_treeview, unique_id, entries
...
def save_details(name, ruc, id_number, lawyer, judge, titulo_credito, concepto, valor_capital, valor_liquidacion, detail_treeview):
    ...
    detail_treeview.insert('', 'end', values=(titulo_credito, name, ruc, id_number, concepto, f"{valor_capital:.2f}".replace('.', ','), f"{valor_liquidacion:.2f}".replace('.', ','), valor_30, lawyer, judge))
    ...
    try:
        ...
        workbook = openpyxl.Workbook()
        sheet = workbook.active
        headers = ["ID", "Título de Crédito", "Name", "RUC", "ID Number", "Concepto", "Valor Capital", "Valor Liquidacion", "Valor 30%", "Lawyer", "Judge"]
        for col, header in enumerate(headers, start=1):
            sheet[get_column_letter(col) + '1'] = header

    # Añadir los nuevos datos al final del archivo
    new_row = [unique_id, titulo_credito, name, ruc, id_number, concepto, f"{valor_capital:,.2f}".replace('.', ','), f"{valor_liquidacion:,.2f}".replace('.', ','), valor_30, lawyer, judge]
    sheet.append(new_row)

    # Nuevo código para actualizar details_storage
    if unique_id not in details_storage:
        details_storage[unique_id] = []
    details_storage[unique_id].append((titulo_credito, concepto, valor_capital, valor_liquidacion,valor_30, lawyer, judge))

    workbook.save(new_excel_path)

root = tk.Tk()
root.title("TABLA PRINCIPAL ATIENCIA & ASOCIADOS - PERSONA NATURAL")
root.state('zoomed')
...
delete_button.grid(row=11, column=0, padx=5, pady=(5, 10), sticky="ew")

# Botón para editar
edit_button = ttk.Button(widgets_frame, text="Editar", command=lambda: edit_row(treeview))
edit_button.grid(row=12, column=0, padx=5, pady=(5, 10), sticky="ew")

# Boton Guardar Cambios

save_button = ttk.Button(widgets_frame, text="Guardar Cambios", command=lambda: save_changes(treeview))
save_button.grid(row=13, column=0, padx=10, pady=(5, 10), sticky="ew")
...
col_widths = [250, 250, 95, 65, 250]
treeview = ttk.Treeview(treeFrame, yscrollcommand=treeScroll.set, columns=cols, height=13, show="headings")
