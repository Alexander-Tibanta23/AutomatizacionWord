...
def load_data():
    path = "data/basedatosPrueba.xlsx"
...
def load_detail_data(id_number):
    new_excel_path = "data/detallesBasedatosPrueba.xlsx"
    try:
...
def update_excel_with_details(id_number, new_values):
    filepath = "data/detallesBasedatosPrueba.xlsx"
    try:
...
def open_details_window(judge, name, ruc, id_number, lawyer):
    ...
    def update_detail_in_excel(id_number, old_detail, new_detail):
        filepath = "data/Providencias/detallesBasedatosPrueba.xlsx"
        wb = load_workbook(filename=filepath)
        sheet = wb.active
    ...
    def update_detail(detail_treeview, id_number, entries):
        ...
        if selected_item:
            ...
            new_values = [entries[0].get(), name, ruc, id_number, entries[1].get(), new_valor_capital, new_valor_liquidacion, new_valor_30, lawyer, judge]  # Assuming 'name', 'ruc', 'lawyer' are accessible here
...
def save_details(name, ruc, id_number, lawyer, judge, titulo_credito, concepto, valor_capital, valor_liquidacion, detail_treeview):
    ...
    try:
        ...
        workbook = openpyxl.Workbook()
        sheet = workbook.active
        headers = ["Título de Crédito", "Name", "RUC", "ID Number", "Concepto", "Valor Capital", "Valor Liquidacion", "Valor 30%", "Lawyer", "Judge"]
        for col, header in enumerate(headers, start=1):
            sheet[get_column_letter(col) + '1'] = header

    # Añadir los nuevos datos al final del archivo
    new_row = [titulo_credito, name, ruc, id_number, concepto, f"{valor_capital:,.2f}".replace('.', ','), f"{valor_liquidacion:,.2f}".replace('.', ','), valor_30, lawyer, judge]
